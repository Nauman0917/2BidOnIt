                    ...
                    <div class="col-md-10 col-xs-12">
                        ...
                        <form action="" id="adsPostForm" class="form-horizontal" method="post"
                            ...
                            <div class="form-group {{ $errors->has('ad_title') ? 'has-error' : '' }}">
                                <label for="ad_title" class="col-sm-4 control-label"><span class="ad_text">
                                        @lang('app.ad') </span> @lang('app.title')</label>
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="ad_title"
                                        value="{{ old('ad_title') ? old('ad_title') : $ad->title }}" name="ad_title"
                                        placeholder="@lang('app.ad_title')">
                            ...
                            <input type="hidden" class="auction_id" value="{{ $ad->id }}">

                            @if (count($auctionedItems) > 0)
                                <div style="display: flex; justify-content: end">
                                    <button class="btn btn-danger" id="remove-items-button" type="button">Return</button>
                                </div>
                                <div class="table-responsive items-table">
                                    <table class="table">
                                        <thead>
                                            <tr>
                                                <th><input type="checkbox" id="select-all-checkbox-remove"></th>
                                                <th>Item Type</th>
                                                <th>Item Name</th>
                                                <th>Start Price</th>
                                            </tr>
                                        ...
                                        <tbody id="selected-items">
                                            @foreach ($auctionedItems as $item)
                                                <tr data-item-id="{{ $item->id }}">
                                                    <td>
                                                        <input type="checkbox" class="remove-item-checkbox"
                                                            value="{{ $item->id }}">
                                                    </td>
                                                    <td>{{ $item->lotItem->item_type }}</td>
                            ...
                            <div style="display: flex; justify-content: end">
                                <button class="btn btn-primary" id="add-items-button" type="button">Add</button>
                            </div>
                            <div class="table-responsive items-table">
                                <table class="table">
                                    <thead>
                                                ...
                                                <tr data-item-id="{{ $item->id }}"
                                                    ...
                                                    <td>
                                                        <input type="checkbox" class="item-checkbox"
                                                            value="{{ $item->id }}"
                                                            {{ in_array($item->id, $auctionedItemIds) ? 'checked' : '' }}>
                                                    </td>
                            ...
                            <div class="form-group {{ $errors->has('video_url') ? 'has-error' : '' }}">
                                ...
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="video_url"
                                        value="{{ old('video_url') ? old('video_url') : $ad->video_url }}"
                                    ...
                                    <select class="form-control select2" name="country">
                                        ...
                                        @foreach ($countries as $country)
                                            <option value="{{ $country->id }}"
                                                {{ $ad->country_id == $country->id ? 'selected' : '' }}>
                                ...
                                <div class="job_field">
                                    <div class="form-group  {{ $errors->has('is_any_where') ? 'has-error' : '' }}">
                                        ...
                                        <div class="col-sm-8">
                                            ...
                                            <label>
                                                <input type="checkbox" name="is_any_where" value="1"
                                                    @if ($ad->job->is_any_where == '1') checked="checked" @endif />
                                                @lang('app.is_any_where')
                                            </label>
                                            <p class="text-info">
                            ...
                            <div class="form-group {{ $errors->has('seller_name') ? 'has-error' : '' }}">
                                ...
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="seller_name"
                                        value="{{ old('seller_name') ? old('seller_name') : $ad->seller_name }}"
                            ...
                            <div class="form-group {{ $errors->has('seller_email') ? 'has-error' : '' }}">
                                ...
                                <div class="col-sm-8">
                                    <input type="email" class="form-control" id="seller_email"
                                        value="{{ old('seller_email') ? old('seller_email') : $ad->seller_email }}"
                            ...
                            <div class="form-group {{ $errors->has('seller_phone') ? 'has-error' : '' }}">
                                ...
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="seller_phone"
                                        value="{{ old('seller_phone') ? old('seller_phone') : $ad->seller_phone }}"
                            ...
                            <div class="form-group {{ $errors->has('address') ? 'has-error' : '' }}">
                                ...
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="address"
                                        value="{{ old('address') ? old('address') : $ad->address }}" name="address"
...
@section('page-js')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            // Select all selected items for removal
            $('#select-all-checkbox-remove').change(function() {
                const isChecked = $(this).is(':checked');
                $('#selected-items .remove-item-checkbox').prop('checked', isChecked);
            });
            ...
            $('#add-items-button').click(function(event) {
                ...
                $('#available-items .item-checkbox:checked').each(function() {
                ...
                if (selectedItems.length > 0) {
                    $.ajax({
                        ...
                        error: function(xhr, status, error) {
                            toastr.error('@lang('app.error_adding_auction_item')');
                        }
                    });
                } else {
                    toastr.error('@lang('app.atleast_select_one_item')');
                }
            });

            // Remove items from the auction
            $('#remove-items-button').click(function(event) {
                event.preventDefault();
                let selectedItems = [];

                $('#selected-items .remove-item-checkbox:checked').each(function() {
                    selectedItems.push($(this).val());
                });
                ...
                if (selectedItems.length > 0) {
                    $.ajax({
                        url: '{{ route('auction_remove_items') }}',
                        method: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            itemIds: selectedItems,
                        ...
                        error: function(xhr, status, error) {
                            toastr.error('@lang('app.error_removing_auction_item')');
                        }
                    });
                } else {
                    toastr.error('@lang('app.atleast_select_one_item')');
