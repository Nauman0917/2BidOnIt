...
function generate_bar_code($name, $description, $item_id, $disk, $bar_code_folder)
{
    // Shorten the description to 50 characters
    $shortDescription = Str::limit($description, 30);

    // Construct the dynamic URL for the item
    $dynamicUrl = url('/dashboard/u/posts/view/item/' . $item_id);

    // Combine the short description with the dynamic URL
    $barcodeData = $name . ': ' . $shortDescription . ' . For more info you can visit: ' . $dynamicUrl;

    $barcodeData = $name . '-' . str_replace(' ', '_', $description);
    ...
    $fileName = $bar_code_folder . '/barcodes/' . $serialNumber . '.png';

    Storage::disk($disk)->put($fileName, base64_decode($barcodeImage));
